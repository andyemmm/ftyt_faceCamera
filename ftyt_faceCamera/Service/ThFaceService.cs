...
namespace ftyt_faceCamera.Service
{
    public class ThFaceService : IThFaceService
    {
        ...
        // 两个人脸特征比对出相似度
        public byte IdFaceFeatureCompare(nint pFeature1, nint pFeature2)
        {
            return IdFaceSdkFeatureCompare(pFeature1, pFeature2);
        }

        // 检测多人脸同时提取各人脸的特征（nMaxFace 表示最多要检测的人脸个数，Faces 必须按最大人脸个数分配人脸坐标空间， pFeatures 必须按最大人脸个数分配特征码空间，pFeatures 参数传 0 时则只检测人脸不提特征）
        public int IdFaceFaceFeature(nint pImage, int nWidth, int nHeight, int nMaxFace, [Out] FACE_DETECT_RESULT[] Faces, [Out] nint pFeatures)
        {
        ...
        // 活体检测并输出活检分数（可根据活检分数是否达到阈值判别是否为活体），需传入人脸检测返回的人脸坐标，pImageColor 与 pImageBW 均有效则进行双目活体检测，如 pImageBW 为 0 则进行彩色单目活体检测，pImageColor 为 0 则进行红外双目活体检测
        public int IdFaceLiveFaceDetectEx(int nWidth, int nHeight, nint pImageColor, ref FACE_DETECT_RESULT FaceColor, nint pImageBW, ref FACE_DETECT_RESULT FaceBW, ref int nScore)
        {
            return IdFaceSdkLiveFaceDetectEx(nWidth, nHeight, pImageColor, ref FaceColor, pImageBW, ref FaceBW, ref nScore);
        }

        // 创建一对多人脸比对列表
        public nint IdFaceListCreate(int nMaxFeatureNum)
        {
            return IdFaceSdkListCreate(nMaxFeatureNum);
        }

        // 向人脸比对列表中增加/插入模板的人脸特征
        public int IdFaceListInsert(nint hList, [In, Out] ref int nPos, nint pFeatures, int nFeatureNum)
        {
            return IdFaceSdkListInsert(hList, ref nPos, pFeatures, nFeatureNum);
        }

        // 从人脸比对列表中删除部分人脸特征
        public int IdFaceListRemove(nint hList, int nPos, int nFeatureNum)
        {
            return IdFaceSdkListRemove(hList, nPos, nFeatureNum);
        }

        // 清空人脸比对列表中的所有人脸特征
        public void IdFaceListClearAll(nint hList)
        {
            IdFaceSdkListClearAll(hList);
        }

        // 一对多人脸比对，返回参与比对的特征数，pnScores 需分配不小于模板特征数的空间，调用后将输出与每个模板特征比对的结果（特征相似度）
        public int IdFaceListCompare(nint hList, nint pFeature, int nPosBegin, int nFeatureNum, [Out] nint pnScores)
        {
            return IdFaceSdkListCompare(hList, pFeature, nPosBegin, nFeatureNum, pnScores);
        }

        // 销毁一对多特征比对列表
        public void IdFaceListDestroy(nint hList)
        {
            IdFaceSdkListDestroy(hList);
        }

        public int IdFaceReadImageFile(nint filename, [Out] nint pRgbBuf, int nBufSize, ref int nWidth, ref int nHeight, int nDepth)
        {
            return ReadImageFile(filename, pRgbBuf, nBufSize, ref nWidth, ref nHeight, nDepth);
        }

        public int IdFaceReadImageFileData(nint pFileData, int nFileDataSize, [Out] nint pRgbBuf, int nBufSize, ref int nWidth, ref int nHeight, int nDepth)
        {
            return ReadImageFileData(pFileData, nFileDataSize, pRgbBuf, nBufSize, ref nWidth, ref nHeight, nDepth);
        }

        public int IdFaceRotateRgb24Data(nint pSrc, int nWidth, int nHeight, int nDegree, int nMirror, [Out] nint pDst)
        {
            return RotateRgb24Data(pSrc, nWidth, nHeight, nDegree, nMirror, pDst);
        }

        public int IdFaceSaveJpegFile(nint filename, nint pRgbData, int nWidth, int nHeight, int nDepth, int nQuality)
        {
            return SaveJpegFile(filename, pRgbData, nWidth, nHeight, nDepth, nQuality);
        }

        public int IdFaceSaveJpegFileData(nint pRgbData, int nWidth, int nHeight, int nDepth, int nQuality, [Out] nint pFileDataBuf, int nBufSize, ref int nFileDataSize)
        {
